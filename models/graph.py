...
class SelfAttentionLayer(nn.Module):
    ...
    def forward(self, h):
        ...
        # attention = F.softmax(e) # Old (torch 1.3.0 deprecated)
        ###############################################################################
        # New (torch 2.3.0+cu118 specify dimension): Added by T.S
        attention = F.softmax(e, dim=-1)  # Typically dim=-1 for attention weights
        ###############################################################################
        # attention = F.dropout(attention, self.dropout, training=self.training)
...
class SpGraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        # e_rowsum = self.special_spmm(edge, edge_e, torch.Size([N, N]), torch.ones(size=(N,1)).cuda()) # Original line based on torch 1.3.0 (avoid this)
        ########################################################## Modernized version based on torch 2.3.0 (recommended) added by T.S
        e_rowsum = self.special_spmm(
            edge.to(DEVICE),
            edge_e.to(DEVICE),
            torch.Size([N, N]),
            torch.ones(size=(N, 1), device=DEVICE)  # Create tensor directly on target device
        )
        ##########################################################
        # e_rowsum: N x 1
