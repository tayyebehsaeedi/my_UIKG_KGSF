...
#     # Change two lines above to following lines below based torch 2.3.0 by T.S
#     #############################################################
#     # sin_pos_enc = torch.FloatTensor(np.sin(position_enc))
#     # cos_pos_enc = torch.FloatTensor(np.cos(position_enc))

#     # # position_encods = torch.zeros_like(out)

#     # # position_encods[:, 0::2] = sin_pos_enc
#     # # position_encods[:, 1::2] = cos_pos_enc
...
######################################################## Change function based torch 2.3.0 by T.S
def create_position_codes(n_pos, dim, out):
    """
    Creates positional encodings and stores them in the provided output tensor.
    
    Args:
        n_pos: Number of positions (sequence length)
        dim: Dimension of the embedding
        out: Output tensor where encodings will be stored (modified in-place)
    """
    ###################################################################################
    set_seed()
    ###################################################################################
    position_enc = np.array([
        [pos / np.power(10000, 2 * j / dim) for j in range(dim // 2)]  # Half-sized
        for pos in range(n_pos)
    ], dtype=np.float32)  # Keep dtype for numerical stability

    with torch.no_grad():  # Disable gradient tracking
        # Convert numpy array to tensor and copy to output (using global DEVICE)
        sin_enc = torch.from_numpy(np.sin(position_enc)).to(device=DEVICE, dtype=out.dtype)
        cos_enc = torch.from_numpy(np.cos(position_enc)).to(device=DEVICE, dtype=out.dtype)

        # Assign values to even and odd indices
        out[:, 0::2] = sin_enc  # dim//2 elements assigned to even indices
        out[:, 1::2] = cos_enc  # dim//2 elements to odd indices

    # Ensure no gradients will be computed
    # out.detach_()
